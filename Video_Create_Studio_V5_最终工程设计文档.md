...
## 10. 长视频 (30min+) 稳定性与性能优化

针对用户反馈的 30 分钟以上超长视频渲染卡死、视频损坏问题，V5.5 引入了专门的稳定性补丁。

### 10.1 硬件加速渲染 (GPU Accel)
支持通过 `--params` 开启硬件加速，大幅降低 CPU 负载并提升导出速度。
- **NVENC**: NVIDIA 显卡加速，编解码器 `h264_nvenc`。
- **QSV**: Intel 核心显卡加速，编解码器 `h264_qsv`。

### 10.2 内存管理优化
- **显式资源释放**：在渲染完成后循环调用 `close_clip()` 确保 FFmpeg 句柄和内存缓冲区被释放。
- **垃圾回收机制**：引入 `gc.collect()` 在关键节点强制回收内存，防止 MoviePy 对象积压导致的 OOM（内存溢出）。

### 10.3 进度心跳与可靠性
- **实时进度反馈**：通过 `JsonMoviePyLogger` 每 1% 进度向 GUI 发送心跳，防止前端因长时间无响应而判定进程死锁。
- **临时文件管理**：使用隔离的 `temp_dir` 存储中间帧和音频流，避免并发冲突。

---

## 11. 版本变更记录 (V5.5)

- **2026-05-14**: 引入 V5.5 章节文字动效系统，支持 6 种视觉模板与 6 种动态动效。
- **2026-05-14**: 引入标签权重系统，实现智能样式推荐。
- **2026-05-14**: 针对长视频渲染稳定性进行专项优化，增加 GPU 加速支持。
